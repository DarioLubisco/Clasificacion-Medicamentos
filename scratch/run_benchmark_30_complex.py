...
def obtener_lote_complejo(limite=30):
    conn = pyodbc.connect(CONN_STR)
    cursor = conn.cursor()
    # Select complex medicines (compounds and syrups)
    query = f"""
    SELECT TOP {limite} codbarras, descrip1art
    FROM Procurement.por_aprobacion_equivalencias 
    WHERE descrip1art LIKE '%jarabe%' 
       OR descrip1art LIKE '%suspensi_n%' 
       OR descrip1art LIKE '%/%/%'
       OR descrip1art LIKE '% mg % mg %'
       OR descrip1art LIKE '%+%'
       OR descrip1art LIKE '% con %'
    ORDER BY NEWID()
    """
...
if __name__ == "__main__":
    print("Obteniendo 30 productos complejos (jarabes, compuestos)...")
    lote = obtener_lote_complejo(30)
    ...
    modelos = [
        ...
        "minimax/minimax-m3",
        "google/gemini-flash-1.5-8b" # as a proxy for fast testing, we can use openrouter models
    ]
    
    # Procesar en lotes de 10 para evitar truncamiento de JSON en modelos propensos a fallar
    chunk_size = 10
    chunks = [lote[i:i + chunk_size] for i in range(0, len(lote), chunk_size)]
    
    resultados = {mod: [] for mod in modelos}
    ...
    for mod in modelos:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if res is None:
                ...
                resultados[mod].extend(res)
            time.sleep(1)
            
        if fallo_modelo:
            resultados[mod] = None
        
    with open('/home/synapse/source/repos/Clasificacion Medicamentos/scratch/benchmark_definitivo_30.json', 'w', encoding='utf-8') as f:
